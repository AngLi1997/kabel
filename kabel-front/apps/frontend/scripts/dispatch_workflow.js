...
function gitlabCiTrigger(nextVersion) {
  ...
  const url = `https://github.com/opendatalab/Kabel-Kit/releases/download/${version}/frontend.zip`;
  const gitlabTriggerUrl = new URL(
    `https://gitlab.shlab.tech/api/v4/projects/${process.env.GI_KABEL_PROJECT_ID}/trigger/pipeline?token=${process.env.GL_TRIGGER_TOKEN}&ref=self-host`,
  );
  ...
  const req = https.request(options, (res) => {
    if (res.statusCode < 300) {
      console.log('trigger kabel workflow success');
    } else {
      console.log('trigger kabel workflow failed', res);
    }
  });

  req.on('error', (e) => {
    console.log('trigger kabel workflow error', e);
  });
...
async function main() {
  ...
  octokit.actions
    ...
    .catch((err) => {
      console.log('trigger kabel workflow failed', err);
    });
