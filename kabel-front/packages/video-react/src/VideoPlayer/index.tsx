...
import type { PlayerControllerRef } from '@kabel/components-react';
import { PlayerController, VIDEO_PLAYBACK_RATE_SPEED } from '@kabel/components-react';
...
const cachedRate = localStorage.getItem('kabel:video-rate') ?? '1';
  ...
  forwardRef<any, React.PropsWithChildren<IProps>>(
    (
      ...
      const handleRateChange = useCallback((value: number) => {
        localStorage.setItem('kabel:rate', value.toString());
        setRate(value);
