...
import type { MediaAnnotatorProps, MediaAnnotatorRef, PlayerControllerRef } from '@kabel/components-react';
import type {
  Attribute,
  AudioAnnotationType,
  AudioFrameName,
...
const cachedRate = localStorage.getItem('kabel:audio-rate') ?? '1';
 ...
 * 音频标注器
 *
 * @see [使用示例](https://github.com/opendatalab/Kabel-Kit/tree/main/packages/audio-react/example)
 */
export const AudioAnnotator = forwardRef<HTMLDivElement, AudioAnnotatorProps>(function ForwardAudioAnnotator(
  ...
  const handleRateChange = useCallback((value: number) => {
    localStorage.setItem('kabel:audio-rate', value.toString());
    setRate(value);
