...
export function StatusCard({ title, active, completed, skipped, children, onClick }: StatusCardProps) {
  ...
  return (
    <StatusCardDiv className="kabel-status-card" active={active} onClick={onClick}>
      <Inner className="kabel-status-card__inner">{children}</Inner>
      {title && <Title className="kabel-status-card__title">{title}</Title>}
      {completed && <CheckIcon />}
      {skipped && <SkipWrapper className="kabel-status-card__skip">{t('skipped')}</SkipWrapper>}
    </StatusCardDiv>
  );
}
