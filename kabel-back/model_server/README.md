...
Kabel 自动标注功能的参考模型服务实现。提供三套方案，均实现统一的 API 协议。
...
# Florence-2
cd model_server/florence2
docker build -t kabel-florence2 .
docker run -p 5000:5000 kabel-florence2

# GroundingDINO + SAM ViT-B (GPU)
cd model_server/grounding_dino_sam
docker build -t kabel-dino-sam .
docker run --gpus all -p 5000:5000 kabel-dino-sam python server.py --device cuda

# SAM 3 (GPU, CUDA 12.6+)
cd model_server/sam3
docker build -t kabel-sam3 .
docker run --gpus all -p 5000:5000 kabel-sam3
```

## 配置 Kabel 连接

在 Kabel 的 `.env` 或环境变量中设置：
...
如果要接入其他模型，只需实现上述 API 协议即可。Kabel 后端通过 HTTP POST 调用模型服务，不感知具体模型实现。
