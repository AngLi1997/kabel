...
class KabelException(HTTPException):
    def __init__(
...
async def kabel_exception_handler(request: Request, exc: KabelException):
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    ...
    if (
        ...
        return FileResponse(
            os.path.join(
                str(files('kabel.internal').joinpath('statics')),
                'index.html'
                ),
            status_code=200,
...
def add_exception_handler(app: FastAPI):
    ...
    app.add_exception_handler(KabelException, kabel_exception_handler)
