...
from kabel.alembic_kabel.alembic_kabel_tools import column_exist_in_table
...
def upgrade() -> None:
    ...
    with context.begin_transaction():
        ...
        op.execute(
            text("UPDATE task SET last_sample_inner_id=(SELECT sample_number FROM ( SELECT task_id, COUNT(id) AS sample_number FROM task_sample GROUP BY task_id ) task_sample_number WHERE task.id=task_sample_number.task_id )")
        )
        op.execute(
            text("UPDATE task_sample SET inner_id=(SELECT newid FROM ( SELECT id,  ROW_NUMBER() OVER (PARTITION BY task_id ORDER BY created_at) AS newid FROM task_sample) tmp_sample_inner_id WHERE task_sample.id=tmp_sample_inner_id.id)")
        )
